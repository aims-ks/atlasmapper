...
import au.gov.aims.atlasmapperserver.thread.ThreadLog;
...
public class Proxy extends HttpServlet {
    ...
    */
    ...
    private static synchronized void reloadConfig(ThreadLogger logger, ServletContext servletContext, String clientId) {
        try {
    ...
    private boolean isHostAllowed(ThreadLogger logger, String clientId, String rawHost) {
        ...
        if (FileFinder.PUBLIC_FOLDER.equals(clientId)) {
            if (allAllowedHostCache == null) {
                reloadConfig(logger, this.getServletContext(), clientId);
            }
            if (allAllowedHostCache == null) {
                return false;
            }
        ...
        ThreadLogger logger = new ThreadLogger() {
            public void log(ThreadLog log) {
                ...
                if (log.getException() != null) {
                    LOGGER.log(log.getLevel(), log.getMessage(), log.getException());
                } else {
        ...
        if (urlStr != null && !urlStr.isEmpty()) {
            ...
            try {
                ...
                if (url != null) {
                    ...
                    String host = url.getHost();
                    if (!isHostAllowed(logger, clientId, host)) {
